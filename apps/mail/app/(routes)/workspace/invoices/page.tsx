...
export default function WorkspaceInvoicesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const data = await api.workspace.getInvoices.query({
  ...
  const handlePayNow = async (_invoiceId: string) => {
    setPaying(true);
    try {
      // This would initiate Razorpay payment
  ...
  // Calculate pending amount for banner
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700 dark:text-yellow-300">
                You have pending invoices totaling {formatCurrency(pendingAmount.toString())}
              </p>
            </div>
          </div>
          <Button
            onClick={() => {
              ...
              if (pendingInvoice) handlePayNow(pendingInvoice.id);
            }}
